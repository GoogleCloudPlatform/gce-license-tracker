...
namespace Google.Solutions.LicenseTracker.Data.Locator
{
		...
		public partial class DiskTypeLocator : ResourceLocator, IEquatable<DiskTypeLocator>
		{
		...
		public partial class InstanceLocator : ResourceLocator, IEquatable<InstanceLocator>
		{
            public string Zone { get; private set; }
		...
		public partial class MachineTypeLocator : ResourceLocator, IEquatable<MachineTypeLocator>
		{
            public string Zone { get; private set; }
		...
		public partial class NodeTypeLocator : ResourceLocator, IEquatable<NodeTypeLocator>
		{
            public string Zone { get; private set; }
		...
		public partial class ImageFamilyViewLocator : ResourceLocator, IEquatable<ImageFamilyViewLocator>
		{
            public string Zone { get; private set; }
            public override string ResourceType => "imageFamilyViews";

            [JsonConstructor]
		    public ImageFamilyViewLocator(string projectId, string zone, string name)
                : base(projectId, name)
            {
                this.Zone = zone;
            }

		    public ImageFamilyViewLocator(ProjectLocator project, string zone, string name)
                : this(project.ProjectId, zone, name)
            {
            }

            public static ImageFamilyViewLocator FromString(string resourceReference)
            {
                resourceReference = StripUrlPrefix(resourceReference);

                var match = new Regex("(?:/compute/beta/)?projects/(.+)/zones/(.+)/imageFamilyViews/(.+)")
                    .Match(resourceReference);
                if (match.Success)
                {
                    return new ImageFamilyViewLocator(
                        match.Groups[1].Value,
                        match.Groups[2].Value,
                        match.Groups[3].Value);
                }
            ...
            public bool Equals(ImageFamilyViewLocator? other)
            {
                return other is object &&
                    this.Name == other.Name &&
                    this.Zone == other.Zone &&
            ...
            public override bool Equals(object? obj)
            {
                return obj is ImageFamilyViewLocator locator && Equals(locator);
            }

            public static bool operator ==(ImageFamilyViewLocator? obj1, ImageFamilyViewLocator? obj2)
            {
                if (obj1 is null)
                {
                    return obj2 is null;
